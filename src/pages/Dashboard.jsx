...
import { GraduationCap, BookOpen, Clock, Users, ChevronRight, Sparkles, History, Trophy, RotateCcw, Eye } from 'lucide-react';
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    mockTests.forEach((test) => {
      const history = JSON.parse(localStorage.getItem(`testHistory_${test.id}`) || '[]');
      if (history.length > 0) {
        histories[test.id] = history;
      }
    });
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-20 text-center">
          ...
          <div className="animate-fade-in-up stagger-3 mt-10 flex items-center justify-center gap-6 sm:gap-10">
            ...
            <div className="w-px h-10 bg-white/20"></div>
            <div className="text-center">
              <p className="text-2xl sm:text-3xl font-bold text-white">9</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {mockTests.map((test, index) => {
            ...
            return (
              <div
                key={test.id}
                className={`test-card bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm animate-fade-in-up stagger-${index + 1}`}
              >
                <div className="p-5 sm:p-6">
                  ...
                  <p className="text-sm text-slate-500 mb-4">NCET Pattern • Full Syllabus</p>
